...
import { getDefaultTaxRate, upsertInvoice } from '../../services/invoice.service';
...
export const listMyBookings = asyncHandler(async (request: Request, response: Response) => {
...
export const getBooking = asyncHandler(async (request: Request, response: Response) => {
  ...
  if (request.user?.role === 'guest' && !isOwnBooking(request, getGuestId(booking.guest))) {
...
export const createBooking = asyncHandler(async (request: Request, response: Response) => {
  ...
  const guestId = request.user?.role === 'guest' ? request.user.id : request.body.guestId;

  if (!guestId) {
    throw new AppError(400, 'guestId is required for staff-created bookings');
  }

  const totalAmount = await calculateBookingAmount(request.body.roomId, checkIn, checkOut);
...
export const updateBooking = asyncHandler(async (request: Request, response: Response) => {
  ...
  const checkIn = request.body.checkIn ? new Date(request.body.checkIn) : existing.checkIn;
  const checkOut = request.body.checkOut ? new Date(request.body.checkOut) : existing.checkOut;
...
export const checkIn = asyncHandler(async (request: Request, response: Response) => {
  const booking = await BookingModel.findById(request.params.id);
...
export const checkOut = asyncHandler(async (request: Request, response: Response) => {
  const booking = await BookingModel.findById(request.params.id);
  ...
  const cleaner = await UserModel.findOne({ role: 'cleaning_staff', isActive: true }).sort({ createdAt: 1 });
  ...
  await upsertInvoice({
    ...
    lineItems: [{ description: 'Room stay charges', quantity: 1, unitPrice: booking.totalAmount }],
    taxRate,
    discount: 0
  });
...
export const cancelBooking = asyncHandler(async (request: Request, response: Response) => {
  ...
  if (request.user?.role === 'guest' && !isOwnBooking(request, String(booking.guest))) {
    throw new AppError(403, 'You do not have access to this booking');
  }
