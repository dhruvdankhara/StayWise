...
const ensureBookingAccess = async (bookingId: string, userId?: string, role?: string) => {
...
export const getInvoice = asyncHandler(async (request: Request, response: Response) => {
  await ensureBookingAccess(request.params.bookingId, request.user?.id, request.user?.role);
  const invoice = await InvoiceModel.findOne({ booking: request.params.bookingId }).populate('guest', 'name email');
...
export const addCharge = asyncHandler(async (request: Request, response: Response) => {
  const existingInvoice = await InvoiceModel.findOne({ booking: request.params.bookingId });
...
export const recordPayment = asyncHandler(async (request: Request, response: Response) => {
  const booking = await ensureBookingAccess(request.params.bookingId, request.user?.id, request.user?.role);
  const invoice = await InvoiceModel.findOne({ booking: request.params.bookingId });
  ...
  booking.paymentStatus = invoice.paidAmount >= invoice.total ? 'paid' : 'partial';
  await booking.save();
  ...
  sendSuccess(response, { invoice, transaction, paymentStatus: booking.paymentStatus }, 'Payment recorded');
});

export const downloadInvoicePdf = asyncHandler(async (request: Request, response: Response) => {
  await ensureBookingAccess(request.params.bookingId, request.user?.id, request.user?.role);
  const invoice = await InvoiceModel.findOne({ booking: request.params.bookingId }).populate('guest', 'name email');
  ...
  const buffer = await buildPdfBuffer(`Invoice ${invoice.id}`, [
    ...
    ...invoice.lineItems.map((item) => `${item.description} - ${item.quantity} x ${item.unitPrice} = ${item.total}`)
  ]);

  response.setHeader('Content-Type', 'application/pdf');
  response.setHeader('Content-Disposition', `attachment; filename=invoice-${request.params.bookingId}.pdf`);
